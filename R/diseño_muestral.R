...
# electorales
basetux <- read_csv("Tuxtla/inp/2018_SEE_AYUN_CHIS_CAS.csv") 
basetux <- basetux %>% rename("PANAL" = "X19")

#Base con los estratos 
dm <- read_csv("Tuxtla/estratos.csv")
...
manzanas_estrato %>%  filter(estrato_final == 3, STATUS == 2 ) %>% 
  ggplot() + geom_sf()

saveRDS(manzanas_estrato, "~/Desktop/manzanas_estrato.rds")
votgeodm %>%  write_excel_csv("~/Desktop/estratos.csv")
